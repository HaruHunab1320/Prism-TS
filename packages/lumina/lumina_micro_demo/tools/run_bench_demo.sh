...
ITERATIONS=${LUMINA_MICRO_DEMO_ITERATIONS:-3}
OUTPUT=${LUMINA_MICRO_DEMO_OUTPUT:-""}
COLD_FIRST=${LUMINA_MICRO_DEMO_COLD_FIRST:-0}
COLD_EACH=${LUMINA_MICRO_DEMO_COLD_EACH:-0}

cmd=(python -m lumina_micro_demo.bench_demo --prompt "$PROMPT" --input "$INPUT" --backend "$BACKEND" --ollama-model "$OLLAMA_MODEL" --ollama-keepalive "$OLLAMA_KEEPALIVE" --iterations "$ITERATIONS")
