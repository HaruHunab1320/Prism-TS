...
from lumina_micro_specialists.evaluation.verify_js_filter_predicate_refactor import verify_js_filter_predicate_refactor
from lumina_micro_specialists.runtime.router_js_filter_predicate_refactor import route_js_filter_predicate_refactor
...
def extract_code(text: str, row: dict) -> str:
    ...
    if match and ".filter(" in match.group(0):
        return " ".join(match.group(0).split())
    for line in stripped.splitlines():
        line = line.strip()
        if row["expected_output_var"] in line and ".filter(" in line and "=" in line:
            return line
    return stripped
...
def main():
    p = argparse.ArgumentParser(description="Evaluate base-model baseline for js_filter_predicate_refactor.")
    p.add_argument("--dataset", type=Path, default=Path("lumina_micro_specialists/data/datasets/js_filter_predicate_refactor_v1/val.jsonl"))
    ...
    routed = 0
    for row in rows:
        decision = route_js_filter_predicate_refactor(row["prompt"], row["input_code"])
        if decision.route != "js_filter_predicate_refactor":
            results.append({"id": row["id"], "routed": False, "correct": False, "syntax_valid": False, "uses_filter": False, "reason": decision.reason})
            continue
        routed += 1
        ...
        verdict = verify_js_filter_predicate_refactor(candidate, row)
        results.append({
            "id": row["id"],
            "routed": True,
            "correct": verdict.passed,
            "syntax_valid": verdict.syntax_valid,
            "uses_filter": verdict.uses_filter,
            "candidate": candidate,
    ...
    payload = {
        ...
        "uses_filter_rate": sum(int(r.get("uses_filter", False)) for r in results) / total if total else 0.0,
        "rows": results,
    }
