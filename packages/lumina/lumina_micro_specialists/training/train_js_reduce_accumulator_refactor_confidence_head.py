...
def main() -> None:
    p = argparse.ArgumentParser(description="Train a confidence probe for js_reduce_accumulator_refactor.")
    p.add_argument("--train-dataset", type=Path, default=Path("lumina_micro_specialists/data/datasets/js_reduce_accumulator_refactor_v1/train.jsonl"))
    p.add_argument("--val-dataset", type=Path, default=Path("lumina_micro_specialists/data/datasets/js_reduce_accumulator_refactor_v1/val.jsonl"))
    ...
    p.add_argument("--output", type=Path, default=Path("lumina_micro_specialists/outputs/js_reduce_accumulator_refactor_confidence_probe.pt"))
    p.add_argument("--metrics-json", type=Path, default=Path("lumina_micro_specialists/notes/js_reduce_accumulator_refactor_confidence_probe_latest.json"))
    args = p.parse_args()
