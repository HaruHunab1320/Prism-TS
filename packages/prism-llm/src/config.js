...
const provider_ai_sdk_1 = require("./provider-ai-sdk");
...
class LLMConfigManager {
    ...
    static createProvider(config) {
        ...
        switch (type) {
            case 'claude': {
                ...
                if (!key) {
                    throw new provider_ai_sdk_1.LLMError('Claude API key not found. Set CLAUDE_API_KEY or ANTHROPIC_API_KEY environment variable.', 'MISSING_API_KEY');
                }
                return new provider_ai_sdk_1.ClaudeProvider(key, options.model);
            }
            case 'gemini': {
                const key = apiKey || this.getApiKey('gemini');
                if (!key) {
                    throw new provider_ai_sdk_1.LLMError('Gemini API key not found. Set GEMINI_API_KEY or GOOGLE_API_KEY environment variable.', 'MISSING_API_KEY');
                }
                return new provider_ai_sdk_1.GeminiProvider(key, options.model);
            }
            case 'mock':
                return new provider_ai_sdk_1.MockLLMProvider();
            default:
                throw new provider_ai_sdk_1.LLMError(`Unknown provider type: ${type}`, 'UNKNOWN_PROVIDER');
        }
    }
    ...
    static validateConfig(config) {
        ...
        if (config.type !== 'mock' && !config.apiKey && !this.getApiKey(config.type)) {
