...
describe('LLM Integration Tests', () => {
    ...
    describeIfApiKey('GeminiProvider - Real API Calls', () => {
        ...
        it('should generate response with confidence using structured output', async () => {
            const request = new src_1.LLMRequest('What is the capital of France?');
        ...
        it('should include reasoning when requested', async () => {
            const request = new src_1.LLMRequest('Is it safe to eat raw cookie dough?', { includeReasoning: true });
            const response = await provider.complete(request);
        ...
        it('should handle uncertain responses with lower confidence', async () => {
            const request = new src_1.LLMRequest('What will the stock market do tomorrow?', { includeReasoning: true });
            const response = await provider.complete(request);
            ...
            const hasUncertaintyIndicator = reasoning.includes('uncertain') ||
        ...
        it('should handle factual questions with high confidence', async () => {
            const request = new src_1.LLMRequest('What is 2 + 2?', { includeReasoning: true });
            const response = await provider.complete(request);
        ...
        it('should work without structured output when disabled', async () => {
            const request = new src_1.LLMRequest('Tell me a joke about programming', { structuredOutput: false });
            const response = await provider.complete(request);
    ...
    describeIfApiKey('Confidence Extraction Integration', () => {
        ...
        it('should extract confidence from complex reasoning', async () => {
            const request = new src_1.LLMRequest('Can you explain quantum entanglement and how certain you are about your explanation?', { includeReasoning: true });
            const response = await provider.complete(request);
        ...
        it('should handle multiple questions with varying confidence', async () => {
            ...
            const responses = await Promise.all(questions.map(q => provider.complete(new src_1.LLMRequest(q))));
            console.log('Multiple questions:', questions.map((q, i) => ({
                question: q,
    ...
    describeIfApiKey('Registry Integration', () => {
        ...
        beforeAll(() => {
            ...
            registry.register('gemini', new src_1.GeminiProvider(apiKey));
            registry.setDefault('gemini');
        });
        it('should complete requests through registry', async () => {
            const request = new src_1.LLMRequest('What is TypeScript?');
