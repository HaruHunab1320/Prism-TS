...
describe('LLM Providers', () => {
    describe('MockLLMProvider', () => {
        let provider;
        beforeEach(() => {
            provider = new src_1.MockLLMProvider();
        });
        it('should have correct name', () => {
            expect(provider.name).toBe('Mock');
        });
        it('should complete requests with default response', async () => {
            const request = new src_1.LLMRequest('Hello AI!');
        ...
        it('should allow setting custom mock responses', async () => {
            ...
            const request = new src_1.LLMRequest('Test prompt');
            const response = await provider.complete(request);
        ...
        it('should simulate failures when failure rate is set', async () => {
            ...
            const request = new src_1.LLMRequest('Test prompt');
            await expect(provider.complete(request)).rejects.toThrow(src_1.LLMError);
        });
        ...
        it('should timeout if latency exceeds request timeout', async () => {
            ...
            const request = new src_1.LLMRequest('Test prompt', { timeout: 100 });
    ...
    describe('ClaudeProvider', () => {
        it('should require API key', () => {
            expect(() => new src_1.ClaudeProvider('')).toThrow('API key is required');
        });
        it('should have correct name', () => {
            const provider = new src_1.ClaudeProvider('test-key');
    ...
    describe('GeminiProvider', () => {
        it('should require API key', () => {
            expect(() => new src_1.GeminiProvider('')).toThrow('API key is required');
        });
        it('should have correct name', () => {
            const provider = new src_1.GeminiProvider('test-key');
    ...
    describe('LLMProviderRegistry', () => {
        ...
        let mockProvider;
        beforeEach(() => {
            registry = new src_1.LLMProviderRegistry();
            mockProvider = new src_1.MockLLMProvider();
        });
    ...
    describe('LLMConfigManager', () => {
        it('should create providers from environment', () => {
            ...
            try {
                const providers = src_1.LLMConfigManager.createFromEnvironment();
        ...
        it('should determine default provider based on available API keys', () => {
            ...
            expect(src_1.LLMConfigManager.getDefaultProvider()).toBe('mock');
            // Test with Claude key
            process.env.CLAUDE_API_KEY = 'test-key';
            expect(src_1.LLMConfigManager.getDefaultProvider()).toBe('claude');
            // Test with only Gemini key (remove Claude key first)
            delete process.env.CLAUDE_API_KEY;
            process.env.GEMINI_API_KEY = 'test-key';
            expect(src_1.LLMConfigManager.getDefaultProvider()).toBe('gemini');
    ...
    describe('LLMRequest', () => {
        ...
        it('should create request with options', () => {
            ...
            const request = new src_1.LLMRequest('Test prompt', options);
            expect(request.prompt).toBe('Test prompt');
    ...
    describe('LLMError', () => {
        it('should create error with code and context', () => {
            const error = new src_1.LLMError('Test error', 'TEST_CODE', { detail: 'value' });
    ...
    describe('Structured Output', () => {
        ...
        it('should return confidence with responses by default', async () => {
            ...
            const request = new src_1.LLMRequest('Tell me something');
            const response = await provider.complete(request);
        ...
        it('should include reasoning when requested', async () => {
            ...
            const request = new src_1.LLMRequest('What is the answer?', {
        ...
        it('should handle low confidence responses', async () => {
            ...
            const request = new src_1.LLMRequest('Tell me something uncertain');
            const response = await provider.complete(request);
