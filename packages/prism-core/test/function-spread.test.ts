...
describe('Function Argument Spread', () => {
  ...
  describe('Spread in function calls', () => {
    ...
    it('should preserve confidence values when spreading', async () => {
      const program = parse(`
        let confident_nums = [10 ~> 0.9, 20 ~> 0.8, 30 ~> 0.7]
        max(...confident_nums)
      `);
      const result = await runtime.execute(program);
  ...
  describe('Integration with other features', () => {
    it('should work in ternary expressions', async () => {
      const program = parse(`
        let useSpread = true
        let nums = [10, 20, 30]
        let result = useSpread ? max(...nums) : max(nums[0])
        result
      `);
      const result = await runtime.execute(program);
    ...
    it('should work in confident operations', async () => {
      const program = parse(`
        let nums = [5, 10, 15]
        let confidentMax = max(...nums) ~> 0.95
        confidentMax
      `);
      const result = await runtime.execute(program);
    ...
    it('should work with optional chaining', async () => {
      const program = parse(`
        let data = { values: [7, 14, 21] }
        let result = data?.values && max(...data.values)
        result
      `);
      const result = await runtime.execute(program);
