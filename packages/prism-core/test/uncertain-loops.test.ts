...
describe('Uncertain Loop Statements', () => {
  ...
  describe('Uncertain For Loops', () => {
    ...
    it('should handle confident condition values', async () => {
      const code = `
        ...
        let i = 0
        uncertain for let j = 0; (j < 1) ~> 0.9; j = j + 1 {
          high {
            results = [...results, "high"]
          }
        }
        
        uncertain for let j = 0; (j < 1) ~> 0.6; j = j + 1 {
          medium {
            results = [...results, "medium"]
          }
        }
        
        uncertain for let j = 0; (j < 1) ~> 0.3; j = j + 1 {
          low {
            results = [...results, "low"]
          }
        }
    ...
    it('should handle break in uncertain for loop', async () => {
      const code = `
        let count = 0
        uncertain for let i = 0; i < 10; i = i + 1 {
          high {
            count = count + 1
            if (i == 2) break
          }
  ...
  describe('Uncertain While Loops', () => {
    ...
    it('should handle changing confidence levels', async () => {
      const code = `
        let results = []
        let confidences = [0.9, 0.7, 0.4, 0.2]
        let i = 0
  ...
  describe('Uncertain Loops with Dynamic Confidence', () => {
    it('should adapt behavior based on sensor confidence', async () => {
      const code = `
        ...
        let readings = []
        let sensorConfidences = [0.95, 0.85, 0.7, 0.4, 0.2]
        
        for let i = 0; i < sensorConfidences.length; i = i + 1 {
          let confidence = sensorConfidences[i]
          uncertain for let j = 0; (j < 1) ~> confidence; j = j + 1 {
            high {
        ...
        let result = {auto: autoCount, review: reviewCount, alert: alertCount}
        result
      `;
      const result = await execute(code);
    ...
    it('should handle nested uncertain loops', async () => {
      const code = `
        ...
        uncertain for let i = 0; i < 2; i = i + 1 {
          high {
            uncertain for let j = 0; j < 2; j = j + 1 {
              high {
                results = [...results, "HH"]
              }
              low {
